...
Этот документ описывает данные, которые backend извлекает из 12 предоставленных Excel. В `ExcelFileStore.currentUpload` хранятся значения отчётов, а не файлы или `ByteArray`. Расчёт заказа и объединение разных источников в единую товарную карточку здесь не выполняются.
...
|---|---|---|
| `iek` | `IekData` | Шесть отчётов IEK из полей формы с префиксом `iek` |
| `systeme` | `SystemeData` | Шесть отчётов Systeme Electric из полей с префиксом `systeme` |
...
| Есть ли автоматическое объединение/дедупликация? | Нет. Отчёты и исходные строки хранятся отдельно; повторный код не удаляется автоматически |
...
Текущий импорт не добавляет ID клиента, цены продажи, сроки поставки из справочника, страховой запас или рекомендуемое количество заказа. `proposedOrderQuantity` — чтение исходной пустой/заполненной ячейки BB, а не результат нового алгоритма.
...
|---|---|---|
| `iekFiles` | `Int` | Количество принятых файлов IEK: 6 |
| `systemeFiles` | `Int` | Количество принятых файлов Systeme: 6 |
| `totalFiles` | `Int` | Общее количество файлов: 12 |
| `recordCounts` | `Map<String, Int>` | Ключ — имя multipart-поля; значение — число основных строк отчёта |
...
Примеры доступа внутри backend после успешной загрузки:
...
Это внутренние Kotlin-модели. Отдельного GET endpoint для выдачи всех строк текущий контракт загрузки не обещает; будущая таблица фронтенда сможет получить отдельный DTO на основе этих данных.
...
Импорт возвращает 15 замечаний: сохранённые `#N/A` в MOQ IEK, для которых `minimumShipmentQuantity = null`. Пустые ячейки выгрузок 1С, помеченные в XML как error-type, но не имеющие значения, читаются как `null` без придуманной ошибки. Встроенный справочный external-link cache прайса IEK отдельно не импортируется. Пройдены 20 тестов и сборка `bootJar`; таблица выше описывает проверенные исходники, а не фиксированные ожидаемые размеры любого нового файла.

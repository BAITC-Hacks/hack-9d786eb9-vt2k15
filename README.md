...
После загрузки данных вызвать `GET /api/orders` с обязательными `leadDays` (0–365) и `reviewDays` (1–365). Сроки не имеют значений по умолчанию. Необязательные параметры: `historyMonths=12` (6–36), `forecastGrowthPercent=0` (−100…300), `supplierId=iek|systeme`, `category`.

```powershell
# 60 и 30 — пример выбранных пользователем сроков, не дефолты сервиса.
Invoke-RestMethod 'http://localhost:8080/api/orders?leadDays=60&reviewDays=30&supplierId=systeme'
```

Ответ содержит `items` с положительными рекомендациями, `skipped` с причинами невозможности расчёта, `review` с нулевыми заказами, требующими проверки, сводки `suppliers` и `warnings`. Без загруженного снимка возвращается `409`, для отсутствующего поставщика — `404`, для неверных параметров — `400`.
...
Существующий frontend нужно адаптировать отдельно: `GET /api/orders` без `leadDays`/`reviewDays` больше не является корректным вызовом. Дополнительно нужно учитывать `skipped`, `review`, nullable-поля и действующий контракт `POST /api/excel`. Backend не сохраняет утверждение и не отправляет заказ поставщику.

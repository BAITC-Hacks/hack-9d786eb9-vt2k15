...
Наша платформа не просто помогает формировать заказ,
а сама сообщает о том, что необходимо отправить заявку.

Интегрируясь с 1С(или любой другой CRM) она всегда имеет доступ к актуальным данным,
что поможет быстро реагировать на разовые крупные продажи
и оперативно пополнять запасы на складах.

Система учитывает: динамику продаж, тенденции, сроки поставок, цены поставщиков,
объем складов, наличие на складах, товары в пути и юнит экономику.

Каждая крупная продажа высвечивается у менеджера, и он решает учитывать ли её для будущих расчетов,
на случай если он знает что подобные заказы будут стабильными.

А когда товары на исходе, система выбирает поставщика, исходя из цены, 
сроки их доставки и прогнозируемом убытке, для того чтобы выбрать наиболее 
выгодный вариант исходя из большого количества данных
...
# Backend загрузки Excel и расчёта заказов

Hackathon team repository for VT2k15

Текущее приложение: Spring Boot + Kotlin + Gradle, Java 21. Принимает одним запросом 6 Excel-файлов IEK, 6 файлов Systeme Electric или оба комплекта из 12 файлов, разбирает их в типизированные данные и сохраняет результат в памяти. `GET /api/orders` рассчитывает рекомендации с объяснениями по последнему снимку. БД нет.
...
Открывайте **http://localhost:5173** — экран загрузки шлёт файлы на реальный backend (`POST /api/excel`). Backend рассчитывает рекомендации через `GET /api/orders`; экран плана нужно адаптировать к описанному ниже контракту и обязательным параметрам.
...
После загрузки данных вызвать `GET /api/orders` с обязательными `leadDays` (0–365) и `reviewDays` (1–365). Сроки не имеют значений по умолчанию. Необязательные параметры: `historyMonths=12` (6–36), `forecastGrowthPercent=0` (−100…300), `anomalyMultiplier=3` (`BigDecimal`, >1 и ≤100), `supplierId=iek|systeme`, `category`.

```powershell
# 60 и 30 — пример выбранных пользователем сроков, не дефолты сервиса.
Invoke-RestMethod 'http://localhost:8080/api/orders?leadDays=60&reviewDays=30&anomalyMultiplier=3&supplierId=systeme'
```

Разовые продажи определяются отдельно для SKU поставщика в окне полных месяцев: при наличии минимум пяти полных положительных расходных накладных итог документа должен быть строго выше среднего остальных, умноженного на `anomalyMultiplier`. Значение по умолчанию — выбранные пользователем 3. Строки документа складываются со знаком по полной дате и номеру. Очистка месячных продаж с ограничением снизу нулём выполняется до оценки stockout и тренда; общий исходный коэффициент сезонности при этом не пересчитывается.

Ответ содержит `items` с положительными рекомендациями, `skipped` с причинами невозможности расчёта, `review` с нулевыми заказами, требующими проверки, `excludedSales` с исключёнными документами и фактическим уменьшением истории, сводки `suppliers` и `warnings`. `excludedSales` включает успешно рассчитанные товары без нового заказа. Без загруженного снимка возвращается `409`, для отсутствующего поставщика — `404`, для неверных параметров — `400`.

Полный контракт, источники, формулы и ограничения: [Расчёт заказов](docs/order_calculation.md). Сезонность, тренд и возможный stockout учитываются с описанными допущениями; это не заявление о полном выполнении ТЗ. Категории пока используются как метаданные и фильтр. Для IEK остаток на начало месяца используется как оценка; неизвестные данные не скрываются за нулевыми значениями.

Существующий frontend нужно адаптировать отдельно: `GET /api/orders` без `leadDays`/`reviewDays` больше не является корректным вызовом. Дополнительно нужно учитывать `skipped`, `review`, `excludedSales`, nullable-поля и действующий контракт `POST /api/excel`. Backend не сохраняет утверждение и не отправляет заказ поставщику.
...
| Путь | Что это |
| --- | --- |
| `src/main/kotlin/` | REST endpoints, Excel-парсеры, DTO, расчёт рекомендаций и хранение данных в памяти |
...
| `docs/order_calculation.md` | Реализованный контракт расчёта, формулы и ограничения |
...
Этот Python-прототип не вызывается REST-приложением. В backend работает отдельный Kotlin-расчёт, описанный в [docs/order_calculation.md](docs/order_calculation.md); результаты старого прототипа ниже относятся только к нему.

...
Каждый цикл менеджер загружает **по 6 отчётов на поставщика** (Systeme Electric и IEK). Поставщик считается по принципу «всё или ничего»: достаточно загрузить одного целиком (6 файлов) или обоих (12).
...
5. «Отправить на бэкенд» активна, когда целиком заполнен хотя бы один поставщик (6 слотов) и нет лишних или частично заполненных поставщиков. Отправляются только полностью собранные поставщики.
...
| Запрос | Тело | Ответ |
| --- | --- | --- |
| `POST /api/imports` | `multipart/form-data`, 6 или 12 полей: имя = `поставщик.тип` (`systeme.sales_tx`, `iek.moq`, …; поставщики `systeme`, `iek`; типы `sales_tx`, `sales_monthly`, `stock_monthly`, `in_transit`, `seasonality`, `moq`), значение = файл. Каждый поставщик — все 6 отчётов либо ни одного; хотя бы один поставщик обязателен | `ImportResult` с `supplier_id` у каждого файла и замечания |
